...
export const generateTeamReport = (lotNo) => {
    ...
    doc.setFontSize(14);
    doc.text("QMAZE - State Level Technical Symposium ", pageWidth / 2, 16, {
        align: "center"
    });
    doc.setFont("helvetica", "normal", "normal");
    doc.setFontSize(10);
    doc.text(`Lot Number : ${team?.lotNo}`, 15, 30);
    doc.text(`College : ${team?.collegeName}`, 80, 30);
    doc.text(`Department : ${team?.deptName} [${team?.deptType}]`, 15, 35);
    doc.text(`Payment Status : ${team?.paymentStatus}`, 80, 35);
    doc.text(`${team?.name}`, 15, 45);
    doc.text(`${team?.email}`, 15, 50);
    doc.text(`${team?.contactNumber}`, 15, 55);

    doc.setFontSize(12);
    doc.text("Team Members", 15, 65);
...
export const generateEventReport = (data, type) => {
    ...
    const pageWidth =
        doc.internal.pageSize.width || doc.internal.pageSize.getWidth();
    doc.addImage(logo, "PNG", pageWidth / 2.5, 10, 40, 40);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.text("QMAZE - State Level Technical Symposium ", pageWidth / 2, 50, {
        align: "center"
    });
    doc.setFont("helvetica", "normal", "normal");
    doc.setFontSize(12);
...
export const generateNetmazeQuestionsTable = (data, dispatch, messageApi) => {
    let tableData = data?.map((value, index) => {
        return { ...value, deleteData: { levelNumber: value.levelNumber, type: value.type, dispatch, messageApi }, key: index }
    })
    return (
        <Table columns={netmazeQuestionTableColumn} dataSource={tableData} pagination={false} className='w-100 px-0' />
    )
}
...
export const generateNetmazeLeaderboard = (data) => {
    ...
    if (t.length > 0) {
        let temp = t?.sort(function (participantOne, participantTwo) {
            return participantOne.level - participantTwo.level || participantTwo.totalAttempt - participantOne.totalAttempt;
        });
